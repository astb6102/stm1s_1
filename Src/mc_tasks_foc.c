...
#define MOTOR_RS        1.0014f    // 상 저항 (Ω)
#define MOTOR_LS        0.0004f  // 상 인덕턴스 (H)
#define MOTOR_BACK_EMF  0.6f   // 역기전력 상수 (V/rad/s)
// 고정소수점 변환 (정수 연산용)
// Rs * 4096 → Q12 형식 (>> 12로 나누기용)
#define MOTOR_RS_Q12    ((int16_t)(MOTOR_RS * 4096.0f))

// Ls * 32768 → Q15 형식 (>> 15로 나누기용)
#define MOTOR_LS_Q15    ((int16_t)(MOTOR_LS * 32768.0f))

// Ke * 1024 → Q10 형식 (>> 10으로 나누기용)
#define MOTOR_KE_Q10    ((int16_t)(MOTOR_BACK_EMF * 1024.0f))
...
/**
  ...
  */
inline uint16_t FOC_CurrControllerM1(void) //feed forward 추가
{
  qd_t Iqd, Vqd;
  ab_t Iab;
  ...
  int16_t hElAngle;
  uint16_t hCodeError;
  int16_t Vq_PI,Vd_PI,Vq_FF,Vd_FF;
  SpeednPosFdbk_Handle_t *speedHandle;
  speedHandle = STC_GetSpeedSensor(pSTC[M1]);

  //if(abs(FOCVars[0].Iab.a)<4000) FOCVars[0].Iab.a=0;
 // if(abs(FOCVars[0].Iab.b)<400) FOCVars[0].Iab.b=0;
  ...
  // PI 제어기 출력 계산
  Vq_PI = PI_Controller(pPIDIq[M1], (int32_t)(FOCVars[M1].Iqdref.q) - Iqd.q);
 Vd_PI = PI_Controller(pPIDId[M1], (int32_t)(FOCVars[M1].Iqdref.d) - Iqd.d);

  // 전기 각속도 읽기 (dpp: degree per PWM period)
  int16_t omega_elec = SPD_GetInstElSpeedDpp(speedHandle);

  // Feedforward 보상 초기화
  int32_t Vq_FF_temp = 0;
  int32_t Vd_FF_temp = 0;

  // 1. 교차 결합 보상 (Cross-coupling compensation)
  // Vq += ω * Ld * Id
  Vq_FF_temp = ((int32_t)omega_elec * (int32_t)MOTOR_LS_Q15 * (int32_t)Iqd.d) >> 15;

  // Vd -= ω * Lq * Iq
  Vd_FF_temp = -((int32_t)omega_elec * (int32_t)MOTOR_LS_Q15 * (int32_t)Iqd.q) >> 15;

  // 2. 역기전력 보상 (Back-EMF compensation) - q축만
  // Vq += ω * λpm
  Vq_FF_temp += ((int32_t)omega_elec * (int32_t)MOTOR_KE_Q10) >> 10;

  // 3. 저항 강하 보상 (Resistive drop compensation)
  // Vq += Rs * Iq_ref
  Vq_FF_temp += ((int32_t)MOTOR_RS_Q12 * (int32_t)FOCVars[M1].Iqdref.q) >> 12;

  // Vd += Rs * Id_ref
  Vd_FF_temp += ((int32_t)MOTOR_RS_Q12 * (int32_t)FOCVars[M1].Iqdref.d) >> 12;

  // Feedforward 값 포화 방지 (±32767 범위)
  if (Vq_FF_temp > 32767) Vq_FF_temp = 32767;
  if (Vq_FF_temp < -32768) Vq_FF_temp = -32768;
  if (Vd_FF_temp > 32767) Vd_FF_temp = 32767;
  if (Vd_FF_temp < -32768) Vd_FF_temp = -32768;

  Vq_FF = (int16_t)Vq_FF_temp;
  Vd_FF = (int16_t)Vd_FF_temp;

  // 최종 전압 지령 = PI 출력 + Feedforward 보상
  Vqd.q = Vq_PI + Vq_FF*0.005;
  Vqd.d = Vd_PI + Vd_FF*0.005;

#else

  // 기존 코드 (Feedforward 없이 PI만 사용)
  Vqd.q = PI_Controller(pPIDIq[M1], (int32_t)(FOCVars[M1].Iqdref.q) - Iqd.q);
  Vqd.d = PI_Controller(pPIDId[M1], (int32_t)(FOCVars[M1].Iqdref.d) - Iqd.d);

#endif
  if(Vqd.q==0) Vqd.d=0;
 // if(abs(Vqd.d)<300) Vqd.d=0;
