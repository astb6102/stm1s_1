/* USER CODE BEGIN Header */
/**
  ******************************************************************************
  * @file           : main.h
  * @brief          : Header for main.c file.
  *                   This file contains the common defines of the application.
  ******************************************************************************
  * @attention
  *
  * Copyright (c) 2025 STMicroelectronics.
  * All rights reserved.
  *
  * This software is licensed under terms that can be found in the LICENSE file
  * in the root directory of this software component.
  * If no LICENSE file comes with this software, it is provided AS-IS.
  *
  ******************************************************************************
  */
/* USER CODE END Header */

/* Define to prevent recursive inclusion -------------------------------------*/
#ifndef __MAIN_H
#define __MAIN_H

#ifdef __cplusplus
extern "C" {
#endif

/* Includes ------------------------------------------------------------------*/
#include "stm32f4xx_hal.h"
#include "motorcontrol.h"

/* Private includes ----------------------------------------------------------*/
/* USER CODE BEGIN Includes */

/* USER CODE END Includes */

/* Exported types ------------------------------------------------------------*/
/* USER CODE BEGIN ET */

/* USER CODE END ET */

/* Exported constants --------------------------------------------------------*/
/* USER CODE BEGIN EC */

/* USER CODE END EC */

/* Exported macro ------------------------------------------------------------*/
/* USER CODE BEGIN EM */

/* USER CODE END EM */

void HAL_TIM_MspPostInit(TIM_HandleTypeDef *htim);

/* Exported functions prototypes ---------------------------------------------*/
void Error_Handler(void);

/* USER CODE BEGIN EFP */

/* USER CODE END EFP */

/* Private defines -----------------------------------------------------------*/
#define M1_CURR_AMPL_Pin GPIO_PIN_1
#define M1_CURR_AMPL_GPIO_Port GPIOC
#define M1_BUS_VOLTAGE_Pin GPIO_PIN_1
#define M1_BUS_VOLTAGE_GPIO_Port GPIOA
#define motor_temp_Pin GPIO_PIN_2
#define motor_temp_GPIO_Port GPIOC
#define B_HUMID_SENSING_Pin GPIO_PIN_2
#define B_HUMID_SENSING_GPIO_Port GPIOA
#define B_TEMP_SENSING_Pin GPIO_PIN_3
#define B_TEMP_SENSING_GPIO_Port GPIOA
#define M1_HALL_H3_Pin GPIO_PIN_10
#define M1_HALL_H3_GPIO_Port GPIOB
#define M1_PWM_UH_Pin GPIO_PIN_8
#define M1_PWM_UH_GPIO_Port GPIOA
#define M1_PWM_VH_Pin GPIO_PIN_9
#define M1_PWM_VH_GPIO_Port GPIOA
#define M1_PWM_WH_Pin GPIO_PIN_10
#define M1_PWM_WH_GPIO_Port GPIOA
#define M1_PWM_EN_U_Pin GPIO_PIN_10
#define M1_PWM_EN_U_GPIO_Port GPIOC
#define M1_PWM_EN_V_Pin GPIO_PIN_11
#define M1_PWM_EN_V_GPIO_Port GPIOC
#define M1_PWM_EN_W_Pin GPIO_PIN_12
#define M1_PWM_EN_W_GPIO_Port GPIOC
#define M1_HALL_H1_Pin GPIO_PIN_8
#define M1_HALL_H1_GPIO_Port GPIOB
#define M1_HALL_H2_Pin GPIO_PIN_9
#define M1_HALL_H2_GPIO_Port GPIOB

/* USER CODE BEGIN Private defines */
extern CAN_HandleTypeDef hcan2;
/* USER CODE END Private defines */

#ifdef __cplusplus
}
#endif

#endif /* __MAIN_H */
